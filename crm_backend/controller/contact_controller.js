     ...
     const contact = await ContactService.existContact(email);
        if(!contact){
            // Create new contact ya babaaaaaaaa
             const newContact = await ContactService.addContact(  email,second_email,name, adress,identity,phones,website,other_info,type  );
               res.status(201).json({
      status: true,
      success: " $type added successfully",
            ...
            await ContactService.updateContact(email,second_email,name, adress,identity,phones,website,other_info,type );
