...
exports.addOrUpdateTask = async (req, res) => {
  try {
    const {id,owner,title,type,revenue,cost,phone,email,isMeet,relatedToNames,relatedToIds,dueDate,time,endTime,address,website,description,status,isPined,} = req.body;
    ...
    if (task == null) {
      // Create new task
      await TaskService.addTask(owner,title,type,revenue,cost,phone,email,isMeet,relatedToNames,relatedToIds,dueDate,time,endTime,address,website,description,status,isPined);
      res.status(201).json({
        status: true,
